...
neighs elev (y, x) = filter (\c -> inBounds c && elevOK c) cands
  where cands = [(y - 1, x), (y, x - 1), (y, x + 1), (y + 1, x)]

        inBounds (yy, xx) = minY <= yy && yy <= maxY && minX <= xx && xx <= maxX
        ((minY, minX), (maxY, maxX)) = bounds elev

        elevOK pos = elev ! pos >= current - 1
        current = elev ! (y, x)
